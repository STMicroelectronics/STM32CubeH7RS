...
/**
  ******************************************************************************
  * @file    Examples_MIX/DMA/DMA_FLASHToRAM/Src/stm32h7rsxx_it.c
...
void HPDMA1_Channel12_IRQHandler(void)
{
  ...
  /* - LL_DMA_IsActiveFlag_TC() to check complete HPDMA1 Interrupt */
  /* - LL_DMA_IsActiveFlag_DTE() to check error HPDMA1 Interrupt   */
  /* - LL_DMA_ClearFlag_TC() to clear all HPDMA1 Interrupts        */

  if(LL_DMA_IsActiveFlag_TC(HPDMA1, LL_DMA_CHANNEL_12) == 1)
  {
    LL_DMA_ClearFlag_TC(HPDMA1, LL_DMA_CHANNEL_12);
    TransferComplete();
  }
  else if(LL_DMA_IsActiveFlag_DTE(HPDMA1, LL_DMA_CHANNEL_12) == 1)
  {
    LL_DMA_ClearFlag_DTE(HPDMA1, LL_DMA_CHANNEL_12);
    TransferError();
  }
  else if(LL_DMA_IsActiveFlag_USE(HPDMA1, LL_DMA_CHANNEL_12) == 1)
  {
    LL_DMA_ClearFlag_USE(HPDMA1, LL_DMA_CHANNEL_12);
    ConfigError();
  }
