...
static const uint32_t aDecrementScatter_ExpectedResult_Buffer[SCATTER_BUFFER_SIZE] =
{
  0xDDDDDDDD, 0xDDDDDDDD, 0xDDDDDDDD, 0xDDDDDDDD,
...
/* USER CODE BEGIN PFP */
static void DMA_Increment_GatherOperation(void);
...
int main(void)
{
  ...
  /*##-1- Incremented gather DMA transaction #################################*/
  memset((void *)aDST_IncrementGather_Buffer, 0U, sizeof(aDST_IncrementGather_Buffer) );

  DMA_Increment_GatherOperation();
  ...
  memset((void *)aDST_DecrementGather_Buffer, 0U, sizeof(aDST_DecrementGather_Buffer));
...
/**
  * @brief  Increment gather DMA transmit.
  * @param  None
  * @retval None
  */
static void DMA_Increment_GatherOperation(void)
{
  ...
  /* USER CODE BEGIN DMA_Increment_GatherOperation */
 /* Select Callbacks functions called after Half Transfer Complete, Transfer Complete and Transfer Error */
  HAL_DMA_RegisterCallback(&handle_GPDMA1_Channel15, HAL_DMA_XFER_CPLT_CB_ID, TransferComplete);
  HAL_DMA_RegisterCallback(&handle_GPDMA1_Channel15, HAL_DMA_XFER_ERROR_CB_ID, TransferError);
  ...
  /* USER CODE END DMA_Increment_GatherOperation */
