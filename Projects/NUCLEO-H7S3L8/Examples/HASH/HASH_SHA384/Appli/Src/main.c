...
/* USER CODE BEGIN PV */
__ALIGN_BEGIN const uint8_t aInput[] __ALIGN_END = "STM32H7 is a high-performance microcontrollers family based on Arm Cortex-M7 32-bit RISC core. It offers native and installable security services.";

__ALIGN_BEGIN static uint8_t aSHA384Digest[48] __ALIGN_END;
__ALIGN_BEGIN static uint8_t aExpectSHA384Digest[48] __ALIGN_END =  
                                                            {0xdf, 0x21, 0x78, 0xec, 0x50, 0x34, 0x26, 0x7d, 0x8b, 0xfb,
                                                             0xcb, 0xf7, 0x1a, 0x2c, 0xe7, 0xaf, 0x17, 0x65, 0x99, 0xb5,
                                                             0x9c, 0xb5, 0x25, 0xc5, 0x63, 0x26, 0xd2, 0x8a, 0xb7, 0x0d,
                                                             0xc8, 0xec, 0x31, 0xd0, 0x2d, 0xc1, 0x0b, 0x7d, 0x54, 0x52,
                                                             0xab, 0x4a, 0x59, 0x45, 0xf1, 0x74, 0x1f, 0x74};
/* USER CODE END PV */
...
int main(void)
{
  ...
  if (HAL_HASH_Start_IT(&hhash, (uint8_t*)aInput, strlen((char const*)aInput), aSHA384Digest) != HAL_OK)
  {
    Error_Handler();
  }
 ...
 /* Compare computed digest with expected one */
  if(memcmp(aSHA384Digest, aExpectSHA384Digest, sizeof(aExpectSHA384Digest)/sizeof(aExpectSHA384Digest[0])) != 0)
  {
    /* Processing Error */
    Error_Handler();
  }
  ...
  while (1)
  {
    /* computed digest are correct : Turn LD1 on */
    BSP_LED_On(LD1);
    /* USER CODE END WHILE */
