...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define EEPROM_ADDR_23_16          0x00    /* EEPROM M95M04 Address  */
#define EEPROM_ADDR_15_8           0x00    /* EEPROM M95M04 Address  */
#define EEPROM_ADDR_7_0            0x00    /* EEPROM M95M04 Address  */
#define EEPROM_CMD_INDEX              0    /* EEPROM Command index   */
#define EEPROM_CMD_WREN            0x06    /* EEPROM Enable Write op */
#define EEPROM_CMD_RDSR            0x05    /* EEPROM Read Status     */
#define EEPROM_CMD_READ            0x03    /* EEPROM Read Data       */
#define EEPROM_CMD_WRITE           0x02    /* EEPROM Write Data      */
#define EEPROM_CMD_SIZE               1    /* EEPROM size Command    */
#define EEPROM_CMD_ADDR_SIZE          4    /* EEPROM size Cmd + Addr */
#define EEPROM_STATUS_INDEX           0    /* EEPROM Status index    */
#define EEPROM_STATUS_CLEAR        0x00    /* EEPROM Status clear    */
#define EEPROM_LONG_TIMEOUT        1000    /* EEPROM Long Timeout 1s */
...
uint8_t aTxBuffer[] =  "SPI HAL API EEPROM driver example: \
                        This firmware provides a basic example of how to use the SPI HAL API based on Polling and \
                        an associate SPI EEPROM driver to communicate with a MKROE EEPROM 5 Click (ST M95M04) \
                        SPI peripheral is configured in Master transmitter during write operation and in \
                        Master receiver during read operation from SPI EEPROM.";

uint8_t aTxCmd[] = {EEPROM_CMD_WRITE, EEPROM_ADDR_23_16, EEPROM_ADDR_15_8, EEPROM_ADDR_7_0};
...
int main(void)
{
  ...
  /* Set SPI CS to low state to select EEPROM */
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, GPIO_PIN_RESET);

  /* Prepare Write Enable command. To enable Write operation on EEPROM */
  aTxCmd[EEPROM_CMD_INDEX] = EEPROM_CMD_WREN;

  /* Send WREN command to EEPROM */
  if (HAL_SPI_Transmit(&hspi1, (uint8_t *)aTxCmd, EEPROM_CMD_SIZE, EEPROM_LONG_TIMEOUT) != HAL_OK)
  {
    /* Transfer error in transmission process */
    Error_Handler();
  }
  /* Set SPI CS to high state to unselect EEPROM */
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, GPIO_PIN_SET);

  /*##-2- Start writing process ##############################################*/
  /* Prepare Write command. Address already set inside table at declararion */
  aTxCmd[EEPROM_CMD_INDEX] = EEPROM_CMD_WRITE;

  /* Set SPI CS to low state to select EEPROM */
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, GPIO_PIN_RESET);

  /* Send WRITE command to EEPROM */
  if (HAL_SPI_Transmit(&hspi1, (uint8_t *)aTxCmd, EEPROM_CMD_ADDR_SIZE, EEPROM_LONG_TIMEOUT) != HAL_OK)
  {
    /* Transfer error in transmission process */
    Error_Handler();
  }

  /* Write TXBUFFERSIZE to EEPROM */
  if (HAL_SPI_Transmit(&hspi1, (uint8_t *)aTxBuffer, TXBUFFERSIZE, EEPROM_LONG_TIMEOUT) != HAL_OK)
  {
  ...
  /*  Before starting a new communication transfer, you must initiate
      a read status (RDSR) until status is cleared (Write operation ended). */
  /* Prepare RDSR (Read Status) command */
  aTxCmd[EEPROM_CMD_INDEX] = EEPROM_CMD_RDSR;

  /* Set SPI CS to low state to select EEPROM */
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, GPIO_PIN_RESET);

  /* Send RDSR (Read Status) command to EEPROM */
  if (HAL_SPI_Transmit(&hspi1, (uint8_t *)aTxCmd, EEPROM_CMD_SIZE, EEPROM_LONG_TIMEOUT) != HAL_OK)
  {
    /* Transfer error in transmission process */
    Error_Handler();
  ...
  /* Wait end of Write operation inside EEPROM */
  do
  {
    /* Read Status) from EEPROM */
    if (HAL_SPI_Receive(&hspi1, (uint8_t *)aRxBuffer, EEPROM_CMD_SIZE, EEPROM_LONG_TIMEOUT) != HAL_OK)
    {
      /* Transfer error in transmission process */
      Error_Handler();
    }
  }
  while (aRxBuffer[EEPROM_STATUS_INDEX] != EEPROM_STATUS_CLEAR);

  /* Set SPI CS to high state to unselect EEPROM */
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, GPIO_PIN_SET);

  /*##-4- Start reading process ##############################################*/
  /* Prepare Read command. Address already set inside table at declararion  */
  aTxCmd[EEPROM_CMD_INDEX] = EEPROM_CMD_READ;
  ...
  /* Read RXBUFFERSIZE from EEPROM */
  if (HAL_SPI_Receive(&hspi1, (uint8_t *)aRxBuffer, RXBUFFERSIZE, EEPROM_LONG_TIMEOUT) != HAL_OK)
  {
    /* Writing process Error */
    Error_Handler();
  }
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(Hold_Signal_GPIO_Port, Hold_Signal_Pin, GPIO_PIN_SET);
  ...
  /*Configure GPIO pin : Hold_Signal_Pin */
  GPIO_InitStruct.Pin = Hold_Signal_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
  HAL_GPIO_Init(Hold_Signal_GPIO_Port, &GPIO_InitStruct);
