## <b>PSSI_Master_ComDMA Example Description</b>
How to handle a DMA communication procedure using two boards with PSSI.
...
Master Board :
  ...
  the reception of a buffer from the Slave through the function HAL_PSSI_Receive_DMA(). The Master will then send a
  buffer in response to the Slave using the function HAL_PSSI_Transmit_DMA(). Finally, the buffer received will be
  compared to the expected buffer.

Slave Board :
  The Slave will send a buffer to the Master board with the function HAL_PSSI_Transmit_DMA(). It will then receive a
  buffer from the Master board through the function HAL_PSSI_Receive_DMA(). Finally, the buffer received will be
  compared to the expected buffer.
...
Start_Clock_Generation() before waiting for the reception of a buffer using HAL_PSSI_Receive_DMA(). Once the reception
is done, the Master will send a buffer through HAL_PSSI_Transmit_DMA(). When the transmission is done, the Master will
stop the PWM generation with Stop_Clock_Generation() and the LD1 will light up if the received buffer is the same as
the expected buffer.
On the Slave side, it will send a buffer to the Master with the function HAL_PSSI_Transmit_DMA(). It will then receive
the buffer sent by the Master through HAL_PSSI_Receive_DMA(). After the reception is done, the Slave will compare the
...
### <b>Directory contents</b>
  - PSSI/PSSI_Master_ComDMA/Appli/Inc/stm32h7rsxx_nucleo_conf.h   BSP configuration file
  - PSSI/PSSI_Master_ComDMA/Appli/Inc/stm32h7rsxx_hal_conf.h      HAL configuration file
  - PSSI/PSSI_Master_ComDMA/Appli/Inc/stm32h7rsxx_it.h            PSSI interrupt handlers header file
  - PSSI/PSSI_Master_ComDMA/Appli/Inc/main.h                      Header for main.c module
  - PSSI/PSSI_Master_ComDMA/Appli/Src/stm32h7rsxx_it.c            PSSI interrupt handlers
  - PSSI/PSSI_Master_ComDMA/Appli/Src/main.c                      Main program
  - PSSI/PSSI_Master_ComDMA/Appli/Src/system_stm32h7rsxx.c        stm32h7rsxx system source file
  - PSSI/PSSI_Master_ComDMA/Appli/Src/stm32h7rsxx_hal_msp.c       HAL MSP file
