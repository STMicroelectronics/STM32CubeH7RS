...
#define USBD_PID     0x7753U
#define USBD_PRODUCT_STRING     "STM32 Video Class"
...
uint8_t * USBD_VIDEO_BOSDescriptor(USBD_SpeedTypeDef speed, uint16_t *length);
...
/** BOS descriptor. */
  ...
  #pragma data_alignment=4
#endif /* defined ( __ICCARM__ ) */
__ALIGN_BEGIN uint8_t USBD_VIDEO_BOSDesc[USB_SIZ_BOS_DESC] __ALIGN_END =
{
  0x5,
  USB_DESC_TYPE_BOS,
  0xC,
...
{
  UNUSED(speed);
  *length = sizeof(USBD_VIDEO_DeviceDesc);
  return USBD_VIDEO_DeviceDesc;
}
...
uint8_t * USBD_VIDEO_SerialStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  ...
  *length = USB_SIZ_STRING_SERIAL;
   ...
   * ID */
  Get_SerialNum();
  /* USER CODE BEGIN USBD_VIDEO_SerialStrDescriptor */

  /* USER CODE END USBD_VIDEO_SerialStrDescriptor */
...
/**
  ...
  * @retval Pointer to descriptor buffer
  */
uint8_t * USBD_VIDEO_USR_BOSDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  UNUSED(speed);
  *length = sizeof(USBD_VIDEO_BOSDesc);
  return (uint8_t*)USBD_VIDEO_BOSDesc;
}
#endif /* (USBD_LPM_ENABLED == 1) */
