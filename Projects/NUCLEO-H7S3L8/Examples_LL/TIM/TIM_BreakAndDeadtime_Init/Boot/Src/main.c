...
static void MX_TIM1_Init(void)
{
  ...
  uint32_t Prescaler = __LL_TIM_CALC_PSC(SystemCoreClock/2, 10000000);
  uint32_t Period = __LL_TIM_CALC_ARR(SystemCoreClock/2, Prescaler, 4000*2);
  uint32_t Pulse1 = (Period * 500) / 1000;
  uint32_t Pulse2 = (Period * 250) / 1000;
  uint32_t Pulse3 = (Period * 125) / 1000;
  uint32_t Deadtime = __LL_TIM_CALC_DEADTIME(SystemCoreClock/2, LL_TIM_CLOCKDIVISION_DIV1, 3000);
  ...
  */
  ...
  PB1   ------> TIM1_CH3N
  */
  GPIO_InitStruct.Pin = LL_GPIO_PIN_13|LL_GPIO_PIN_11|LL_GPIO_PIN_9;
  GPIO_InitStruct.Mode = LL_GPIO_MODE_ALTERNATE;
