...
uint32_t tickstart;
static __IO uint32_t RF1Tick = 0;
static __IO uint32_t RF2Tick = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  HAL_GFXTIM_RelativeTimer_Start(&hgfxtim,GFXTIM_RELATIVE_TIMER2);

  tickstart = HAL_GetTick();
  while (((HAL_GetTick() - tickstart) < 2010) && (RF1Tick == 0))
  {
  }
  if (RF1Tick - tickstart == 2000)
  {
    Time1Result = 1;
  }

  while (((HAL_GetTick() - tickstart) < 5010) && (RF2Tick == 0))
  {
  }
  if (RF2Tick - tickstart == 5000)
  {
    Time2Result = 1;
  }
...
static void MX_GFXTIM_Init(void)
{
  ...
  pClockGeneratorConfig.LCCReloadValue = 149999;
  pClockGeneratorConfig.LCCClockSrc = GFXTIM_LCC_CLK_SRC_SYSCLOCK;
  pClockGeneratorConfig.FCCHwReloadSrc = GFXTIM_FCC_HW_RELOAD_SRC_NONE;
  pClockGeneratorConfig.FCCReloadValue = 9;
  pClockGeneratorConfig.FCCClockSrc = GFXTIM_FCC_CLK_SRC_LCC_UNDERFLOW;
  ...
  pRelativeTimerConfig.AutoReloadValue = 199;
  pRelativeTimerConfig.CounterMode = GFXTIM_MODE_ONE_SHOT;
  pRelativeTimerConfig.ReloadInterrupt = ENABLE;
  if (HAL_GFXTIM_RelativeTimer_Config(&hgfxtim, &pRelativeTimerConfig, GFXTIM_RELATIVE_TIMER1) != HAL_OK)
  {
    Error_Handler();
  }
  pRelativeTimerConfig.AutoReloadValue = 499;
  if (HAL_GFXTIM_RelativeTimer_Config(&hgfxtim, &pRelativeTimerConfig, GFXTIM_RELATIVE_TIMER2) != HAL_OK)
  {
    Error_Handler();
  }
