...
uint32_t tickstart;
static __IO uint32_t CF1Tick = 0;
static __IO uint32_t OFTick = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (((HAL_GetTick() - tickstart) < 1050) && (CF1Tick == 0))
  {
  }
  if (CF1Tick - tickstart == 1048)
  {
    Time1Result = 1;
  }

  while (((HAL_GetTick() - tickstart) < 2100) && (OFTick == 0))
  {
  }
  if (OFTick - tickstart == 2097)
  {
    Time2Result = 1;
  }
...
static void MX_GFXTIM_Init(void)
{
  ...
  pAbsoluteTimerConfig.FrameCompare1Value = 524287;
  pAbsoluteTimerConfig.LineCompare1Value = 0;
  pAbsoluteTimerConfig.LineCompare2Value = 0;
  pAbsoluteTimerConfig.FrameCounterValue = 0;
  pAbsoluteTimerConfig.LineCounterValue = 0;
  pAbsoluteTimerConfig.FrameOverflowInterrupt = ENABLE;
  pAbsoluteTimerConfig.FrameCompare1Interrupt = ENABLE;
