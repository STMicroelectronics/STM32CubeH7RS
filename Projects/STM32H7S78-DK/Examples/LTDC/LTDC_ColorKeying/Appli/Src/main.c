...
/* USER CODE BEGIN PV */
/* Private variables ---------------------------------------------------------*/
#define PRESSED_FIRST    	0x01
#define PRESSED_SECOND   	0x00
...
int main(void)
{
  ...
  /*##-2- LCD Configuration ##################################################*/
  /* done by MX_LTDC_Init(); */
  
  /*##-3- Configure Color Keying  ############################################*/
  HAL_LTDC_ConfigColorKeying(&hltdc, ARGB4444_COLOR_KEY, LTDC_LAYER_1);  
  ...
  while (1)
  {
   ...
   /* Simple Polling on user button state */
    ...
    if(ubPressedButton == PRESSED_FIRST)
    {
      ...
      HAL_LTDC_EnableColorKeying(&hltdc, LTDC_LAYER_1);
      ubPressedButton = PRESSED_SECOND;  
    }
    else
    {
      /* Disable Color Keying on layer 1 */
      HAL_LTDC_DisableColorKeying(&hltdc, LTDC_LAYER_1);
...
static void MX_LTDC_Init(void)
{
  ...
  pLayerCfg.WindowX1 = 485;
  pLayerCfg.WindowY0 = 100;
  pLayerCfg.WindowY1 = 330;
