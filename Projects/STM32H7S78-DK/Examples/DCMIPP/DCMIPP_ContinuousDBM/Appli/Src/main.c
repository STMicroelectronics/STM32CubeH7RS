...
/* USER CODE BEGIN PV */
const DCMIPP_EmbeddedSyncCodesTypeDef SynCodes = {0,0,0,0};
__IO uint32_t NumberOfPipe0Frames = 0;
__IO uint32_t ReloadFlag = 0;
uint32_t CAMERA_FRAME_BUFFER[FRAME_BUFFER_SIZE] __attribute__((aligned(16)));
uint32_t CAMERA_FRAME_BUFFER2[FRAME_BUFFER_SIZE] __attribute__((aligned(16)));

uint32_t *Display_pos;
uint32_t xpos, ypos;
uint32_t Request_DisplayUpdate = 0;
uint32_t timeout = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  if(OV5640_Config(OV5640_R320x240, OV5640_RGB565) != OV5640_OK)
  {
  ...
  if(HAL_DCMIPP_PIPE_DoubleBufferStart(&phdcmipp, DCMIPP_PIPE0, (uint32_t)CAMERA_FRAME_BUFFER, (uint32_t)CAMERA_FRAME_BUFFER2  , DCMIPP_MODE_CONTINUOUS) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  while (1)
  {
     if(NumberOfPipe0Frames == 0)
     {
       /* Next 50 Frames will be displayed from the first destination address */
       /* Update Display window and Layer Address */
       /* The Frame will be displayed in the top left side */
       xpos = 0;
       ypos = 0;
       Display_pos = CAMERA_FRAME_BUFFER;
       Request_DisplayUpdate = 1;
     }
     else if(NumberOfPipe0Frames == 50)
     {
       /* Next 50 Frames will be displayed from the second destination address */
       /* Update Display window and Layer Address */
       /* The Frame will be displayed in the right bottom side */
       xpos = 480;
       ypos = 240;
       Display_pos = CAMERA_FRAME_BUFFER2;
       Request_DisplayUpdate = 1;
     }

     if(Request_DisplayUpdate != 0)
     {
       Request_DisplayUpdate = 0;

       HAL_LTDC_SetWindowPosition_NoReload(&hltdc, xpos, ypos, LTDC_LAYER_1);

       HAL_LTDC_SetAddress_NoReload(&hltdc, (uint32_t)Display_pos, LTDC_LAYER_1);

       /*Ask for LTDC reload within next vertical blanking*/
       ReloadFlag = 0;
       HAL_LTDC_Reload(&hltdc,LTDC_RELOAD_VERTICAL_BLANKING);
       while((ReloadFlag == 0) & (timeout < 0xFFFF))
       {
...
static uint32_t OV5640_Config(uint32_t Resolution, uint32_t PixelFormat)
{
  ...
  if(id == OV5640_ID)
  {
    ...
    else if(OV5640_CAMERA_Driver.MirrorFlipConfig(&OV5640Obj, OV5640_MIRROR_FLIP_NONE) != OV5640_OK)
    {
      ret = OV5640_ERROR;
    }
  }
...
void HAL_DCMIPP_PIPE_FrameEventCallback(DCMIPP_HandleTypeDef *pHdcmipp, uint32_t Pipe)
{
  ...
  /* Reset frame number after 100 Frames aqcuisition */
  if (NumberOfPipe0Frames > 100)
  {
    NumberOfPipe0Frames = 0;
  }
