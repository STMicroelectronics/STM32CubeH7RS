...
/* USER CODE BEGIN PV */
uint8_t ALIGN_32BYTES(CAMERA_FRAME_BUFFER[FRAME_BUFFER_SIZE]);
...
int main(void)
{
  ...
  if(OV5640_Config(OV5640_R480x272, OV5640_JPEG) != OV5640_OK)
  {
  ...
  if(HAL_DCMIPP_PIPE_Start(&phdcmipp, DCMIPP_PIPE0, (uint32_t)CAMERA_FRAME_BUFFER , DCMIPP_MODE_SNAPSHOT) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  while(NumberOfPipe0Frames == 0);

  /* Check start of the Jpeg delimiter */
  if ((CAMERA_FRAME_BUFFER[0] == 0xFF) && (CAMERA_FRAME_BUFFER[1] == 0xD8))
  {
    /* Get the transferred data */
    if(HAL_DCMIPP_PIPE_GetDataCounter(&phdcmipp, DCMIPP_PIPE0, &DataCounter) != HAL_OK)
    {
      Error_Handler();
    }

    /* According to the transferred size , check the end jpeg delimiter */
    /* DataCounter = Number of transferred Bytes */
    /* Try to find the end of the frame in the last bytes */
    for(i=DataCounter ; i> 10; i--)
    {
      if(CAMERA_FRAME_BUFFER[i] == 0xD9)
      {
        if(CAMERA_FRAME_BUFFER[i-1] == 0xFF)
        {
          /* End Frame found : Test OK */
          BSP_LED_On(LD1);
          break;
        }
        else
...
static void MX_DCMIPP_Init(void)
{
  ...
  */
  ...
  ParallelConfig.Format = DCMIPP_FORMAT_BYTE;
...
void HAL_DCMIPP_PIPE_FrameEventCallback(DCMIPP_HandleTypeDef *pHdcmipp, uint32_t Pipe)
{
  ...
  /* Invalidate the data cache for the CAMERA_FRAME_BUFFER */
  SCB_InvalidateDCache_by_Addr(CAMERA_FRAME_BUFFER, FRAME_BUFFER_SIZE);
}
/* USER CODE END 4 */
