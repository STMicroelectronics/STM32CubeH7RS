...
set "ld_file=%project_dir%STM32H7S7L8HXH_FLASH.ld"

set "offset_macro=CODE_OFFSET"
set "offset_pattern=CODE_OFFSET ="

for /f "delims=:" %%a in ('findstr /c:"%offset_pattern%" "%ld_file%"') do (
	set found=%%a
)
...
set "size_pattern=CODE_SIZE ="
for /f "delims=:" %%a in ('findstr /c:"%size_pattern%" "%ld_file%"') do (
  set size_found=%%a
)
...
set "command=%python%%applicfg% linker -l "%project_dir%tmp.h" -m %offset_macro% -n %offset_macro% %ld_file% > "%project_dir%output.txt" 2>&1"
%command%

set "command=%python%%applicfg% linker -l "%project_dir%tmp.h" -m %size_macro% -n %size_macro% %ld_file% > "%project_dir%output.txt" 2>&1"
%command%
