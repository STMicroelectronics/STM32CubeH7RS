...
void OPENBL_I2C_Configuration(void)
{
  ...
  /* Common configuration for I2Cx PINs:
      ...
      Speed       : High speed.
  */
  /* Only initialize I2C if it is enabled in "I2C_NI3C" filed of "OBW2SR" option byte */
...
void OPENBL_I2C_SpecialCommandProcess(OPENBL_SpecialCmdTypeDef *p_special_cmd)
{
  ...
  switch (p_special_cmd->OpCode)
  {
    case SPECIAL_CMD_EXT_MEM_INIT:
      ...
      if (status == SUCCESS)
      {
        OPENBL_I2C_SendByte(SPECIAL_CMD_SUCCESS);
      }
      else
      {
        OPENBL_I2C_SendByte(SPECIAL_CMD_ERROR);
      }
