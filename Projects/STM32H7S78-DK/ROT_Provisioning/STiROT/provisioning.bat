...
if /i %appli_dir% == %appli_dir:STiROT_Appli=% (
    echo =====
    echo ===== Wrong Boot path: %appli_dir%
...
echo          Select STiROT_Config.xml (Default path is (Default path is \ROT_Provisioning\STiROT\Config\STiROT_Config.xml)
...
:: ================================================ Updating STiROT Application files ==========================================
echo;
...
if "%isGeneratedByCubeMX%" == "false" (
    set "action=STiROT_Appli project files (linker files, .h) update ..."
    echo    * !action!

    %command% > %ob_update_appli_log% 2>&1
    if !errorlevel! neq 0 goto :step_error

    echo        ^(stm32H7S73xx_flash.icf and main.h successfully updated according to STiROT_Config.xml^)

    echo        ^(see %ob_update_appli_log% for details^)
)

:: ============================================= STiROT application image generation step ======================================
echo;
echo Step 2: STiROT application image generation
echo;
:: bypass this step when Generated By CubeMX
...
set "action=Programming the option bytes and flashing the image ..."
...
echo        Successful option bytes programming and image flashing
