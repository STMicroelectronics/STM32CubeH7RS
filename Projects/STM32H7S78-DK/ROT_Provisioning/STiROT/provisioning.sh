...
if [ "$isGeneratedByCubeMX" == "false" ]; then
  ...
  if [[ "$appli_dir" != *STiROT_Appli* ]]; then
    echo "====="
...
if [ "$isGeneratedByCubeMX" == "false" ]; then
  action="update of STiROT_Appli project files (main.h, .ld) ..."
  echo "   * $action"

  $command > $ob_update_appli_log 2>&1
  if [ $? -ne 0 ]; then step_error; return 1; fi

  echo "       STM32H7S7L8HXH_FLASH.ld and main.h successfully updated according to STiROT_Config.xml"
